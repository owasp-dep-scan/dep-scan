...
from depscan.lib.analysis import (PrepareVdrOptions, analyse_licenses, analyse_pkg_risks, find_purl_usages, prepare_vdr,
                                  summary_stats, )
...
from depscan.lib.config import (UNIVERSAL_SCAN_TYPE, VDB_AGE_HOURS, vdb_database_url, license_data_dir,
                                spdx_license_list, )
from depscan.lib.csaf import export_csaf, write_toml
from depscan.lib.license import build_license_data, bulk_lookup
from depscan.lib.logger import DEBUG, LOG, console, SPINNER
...
def build_parser():
    parser = tomlparse.ArgumentParser(description="Fully open-source security and license audit for "
    ...
    parser.add_argument("--csaf", action="store_true", default=False, dest="csaf",
    ...
    parser.add_argument("--profile", default="generic",
        choices=("appsec", "research", "operational", "threat-modeling", "license-compliance", "generic",
                 "machine-learning", "ml", "deep-learning", "ml-deep", "ml-tiny",), dest="profile",
        help="Profile to use while generating the BOM.", )
    parser.add_argument("--lifecycle", choices=("pre-build", "build", "post-build"), nargs="+", type=str,
        dest="lifecycles", help="Product lifecycle for the generated BOM. Multiple values allowed.", )
    parser.add_argument("--technique",
        choices=("auto", "source-code-analysis", "binary-analysis", "manifest-analysis", "hash-comparison",
                 "instrumentation", "filename"), nargs="+", type=str, dest="techniques",
        help="Analysis technique to use for BOM generation. Multiple values allowed.", )
    parser.add_argument("--bom-engine",
        choices=("auto", "CdxgenGenerator", "CdxgenServerGenerator", "CdxgenImageBasedGenerator", "BlintGenerator"),
        default="auto", dest="bom_engine",
        help="BOM generation engine to use. Defaults to automatic selection based on project type and lifecycle.", )
    parser.add_argument("--no-suggest", action="store_false", default=True, dest="suggest",
        help="Disable suggest mode", )
    parser.add_argument("--risk-audit", action="store_true", default=os.getenv("ENABLE_OSS_RISK", "") in ("true", "1"),
        dest="risk_audit", help="Perform package risk audit (slow operation). Npm only.", )
    parser.add_argument("--cdxgen-args", default=os.getenv("CDXGEN_ARGS"), dest="cdxgen_args",
        help="Additional arguments to pass to cdxgen", )
    parser.add_argument("--private-ns", dest="private_ns", default=os.getenv("PKG_PRIVATE_NAMESPACE"),
    ...
    parser.add_argument("--bom-dir", dest="bom_dir",
        help="Examine all the Bill-of-Materials (BOM) files in the given directory.", )
    parser.add_argument("-i", "--src", dest="src_dir_image",
        help="Source directory or container image or binary file", )
    parser.add_argument("-o", "--reports-dir",
        default=os.getenv("DEPSCAN_REPORTS_DIR", os.path.join(os.getcwd(), "reports")), dest="reports_dir",
        help="Reports directory", )
    parser.add_argument("--report-template", dest="report_template",
        help="Jinja template file used for rendering a custom report", )
    parser.add_argument("--report-name", default="rendered.report", dest="report_name",
        help="Filename of the custom report written to the --reports-dir", )
    parser.add_argument("--deep", action="store_true", default=False, dest="deep_scan",
        help="Perform deep scan by passing this --deep argument to cdxgen. "
             "Useful while scanning docker images and OS packages.", )
    parser.add_argument("--no-universal", action="store_true", default=False, dest="non_universal_scan",
        help="Depscan would attempt to perform a single universal scan "
             "instead of individual scans per language type.", )
    parser.add_argument("--no-vuln-table", action="store_true", default=False, dest="no_vuln_table",
        help="Do not print the table with the full list of vulnerabilities. "
             "This can help reduce console output.", )
    parser.add_argument("--server", action="store_true", default=False, dest="server_mode",
        help="Run depscan as a server", )
    parser.add_argument("--server-host", default=os.getenv("DEPSCAN_HOST", "127.0.0.1"), dest="server_host",
        help="depscan server host", )
    parser.add_argument("--server-port", default=os.getenv("DEPSCAN_PORT", "7070"), dest="server_port",
        help="depscan server port", )
    parser.add_argument("--cdxgen-server", default=os.getenv("CDXGEN_SERVER_URL"), dest="cdxgen_server",
        help="cdxgen server url. Eg: http://cdxgen:9090", )
    parser.add_argument("--debug", action="store_true", default=False, dest="enable_debug",
    ...
    parser.add_argument("--reachables-slices-file", dest="reachables_slices_file",
        help="Path for the reachables slices file created by atom.", )
    parser.add_argument("--purl", dest="search_purl", help="Scan a single package url.", )
    parser.add_argument("-v", "--version", help="Display the version", action="version",
...
def summarise(project_type, results, pkg_aliases, purl_aliases, suggest_mode, scoped_pkgs, report_file, bom_file,
    no_vuln_table=False, direct_purls=None, reached_purls=None, ):
    """
    ...
    """
    options = PrepareVdrOptions(project_type, results, pkg_aliases, purl_aliases, suggest_mode, scoped_pkgs=scoped_pkgs,
        no_vuln_table=no_vuln_table, bom_file=bom_file, direct_purls=direct_purls, reached_purls=reached_purls, )
...
def summarise_tools(tools, metadata, bom_data):
    ...
    components.append(
        {"type": "application", "name": "owasp-depscan", "version": ds_version, "purl": ds_purl, "bom-ref": ds_purl, })
...
if QUART_AVAILABLE:
    ...
    async def download_vdb():
        ...
        if db_lib.needs_update(days=0, hours=VDB_AGE_HOURS, default_status=False):
            ...
            if download_image(vdb_database_url, config.DATA_DIR):
                return {"error": "false", "message": "vulnerability database downloaded successfully", }
            return {"error": "true",
    ...
    async def run_scan():
        ...
        if params is not None:
            ...
            if not deep and params.get("deep"):
                deep = params.get("deep", "").lower() in ("true", "1",)
        ...
        if db_lib.needs_update(days=0, hours=VDB_AGE_HOURS, default_status=False):
            return ({"error": "true", "message": "Vulnerability database is empty. Prepare the "
                                                 "vulnerability database by invoking /download-vdb endpoint "
                                                 "before running scans.", }, 500, {"Content-Type": "application/json"},)
        ...
        if uploaded_bom_file.get("file", None) is not None:
            ...
            try:
                ...
                LOG.info(e)
                return ({"error": "true", "message": "The uploaded file must be a valid JSON or XML.", }, 400,
        ...
        if url or (path and os.path.isdir(path)):
            with tempfile.NamedTemporaryFile(delete=False, suffix=".bom.json") as bfp:
                ...
                bom_status = create_bom(project_type_list, bfp.name, path,
                    {"url": url, "path": path, "type": project_type_list, "multiProject": multi_project,
                        "cdxgen_server": cdxgen_server, "profile": profile, "deep": deep, }, )
                if bom_status:
        ...
        if bom_file_path is not None:
            ...
            if not bom_data:
                return ({"error": "true", "message": "Unable to generate SBOM. Check your input path or url.", }, 400,
                        {"Content-Type": "application/json"},)
            options = PrepareVdrOptions(project_type, results, pkg_aliases, purl_aliases, suggest_mode, scoped_pkgs={},
                no_vuln_table=True, bom_file=bom_file_path, direct_purls={}, reached_purls={}, )
            pkg_vulnerabilities, _ = prepare_vdr(options)
            if pkg_vulnerabilities:
                bom_data["vulnerabilities"] = pkg_vulnerabilities
            return json.dumps(bom_data), 200, {"Content-Type": "application/json"}

        return ({"error": "true", "message": "Unable to generate SBOM. Check your input path or url.", }, 500,
                {"Content-Type": "application/json"},)
    ...
    def run_server(args):
        ...
        app.run(host=args.server_host, port=args.server_port, debug=os.getenv("SCAN_DEBUG_MODE") == "debug",
            use_reloader=False, )
...
def run_depscan(args):
    ...
    if db_lib.needs_update(days=0, hours=VDB_AGE_HOURS, default_status=db_lib.get_db_file_metadata is not None):
        if ORAS_AVAILABLE:
            with console.status(f"Downloading the latest vulnerability database to {config.DATA_DIR}. Please wait ...",
                                spinner=SPINNER):
    ...
    if "license" in project_types_list or "license" in args.profile:
        ...
        console.print(Panel("License audit is enabled for this scan. This would increase "
                            "the time by up to 10 minutes.", title="License Audit", expand=False, ))
    for project_type in project_types_list:
        ...
        if bom_file:
            ...
            if not args.bom:
                LOG.info("To improve performance, cache the bom file and invoke "
        ...
        if (os.getenv("FETCH_LICENSE", "") in (True, "1", "true") or "license" in args.profile):
            licenses_results = bulk_lookup(build_license_data(license_data_dir, spdx_license_list), pkg_list=pkg_list, )
            license_report_file = os.path.join(reports_dir, "license-" + project_type + ".json")
            analyse_licenses(project_type, licenses_results, license_report_file)
        if project_type in risk_audit_map:
            if perform_risk_audit:
                if len(pkg_list) > 1:
                    console.print(Panel(f"Performing OSS Risk Audit for packages from "
                                        f"{src_dir}\nNo of packages [bold]{len(pkg_list)}"
                                        f"[/bold]. This will take a while ...", title="OSS Risk Audit", expand=False, ))
                try:
                    risk_results = risk_audit(project_type, scoped_pkgs, args.private_ns, pkg_list, )
                    analyse_pkg_risks(project_type, scoped_pkgs, risk_results, risk_report_file, )
                except Exception as e:
                    LOG.error(e)
                    LOG.error("Risk audit was not successful")
            else:
                console.print(Panel("Depscan supports OSS Risk audit for this "
                                    "project.\nTo enable set the environment variable ["
                                    "bold]ENABLE_OSS_RISK=true[/bold]", title="Risk Audit Capability", expand=False, ))
        if project_type in type_audit_map:
            LOG.debug("Performing remote audit for %s of type %s", src_dir, project_type, )
        ...
        if len(pkg_list) > 1:
            if args.bom:
                LOG.info("Scanning %s with type %s", args.bom, project_type, )
            else:
                LOG.info("Scanning %s with type %s", src_dir, project_type, )
        if not pkg_list:
            LOG.debug("Empty package search attempted!")
        else:
            LOG.debug("Scanning %d oss dependencies for issues", len(pkg_list))
        vdb_results, pkg_aliases, purl_aliases = utils.search_pkgs(project_type, pkg_list)
        results.extend(vdb_results)
        direct_purls, reached_purls = find_purl_usages(bom_file, src_dir, args.reachables_slices_file)
        # Summarise and print results
        summary, vdr_file, pkg_vulnerabilities, sug_version_dict, pkg_group_rows = summarise(project_type, results,
            pkg_aliases, purl_aliases, args.suggest, scoped_pkgs=scoped_pkgs, report_file=report_file,
            bom_file=bom_file, no_vuln_table=args.no_vuln_table, direct_purls=direct_purls,
            reached_purls=reached_purls, )
        # Explain the results
        if args.explain:
            explainer.explain(project_type, src_dir, args.reachables_slices_file, vdr_file, pkg_vulnerabilities,
                pkg_group_rows, direct_purls, reached_purls, )
        # CSAF VEX export
        if args.csaf:
            export_csaf(pkg_vulnerabilities, src_dir, reports_dir, bom_file, )
    console.save_html(html_file, theme=(MONOKAI if os.getenv("USE_DARK_THEME") else DEFAULT_TERMINAL_THEME), )
    ...
    if args.report_template and os.path.isfile(args.report_template):
        utils.render_template_report(vdr_file=vdr_file, bom_file=bom_file, pkg_vulnerabilities=pkg_vulnerabilities,
            pkg_group_rows=pkg_group_rows, summary=summary, template_file=args.report_template,
