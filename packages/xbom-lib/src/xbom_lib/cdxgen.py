...
DEFAULT_IMAGE_NAME = "default-secure" if os.getenv("CDXGEN_SECURE_MODE", "") in ("true", "1") else "default"

PROJECT_TYPE_IMAGE = {"default": f"ghcr.io/cyclonedx/cdxgen:{CDXGEN_IMAGE_VERSION}",
                      ...
                      "java": f"ghcr.io/cyclonedx/cdxgen-java:{CDXGEN_IMAGE_VERSION}",
                      "android": f"ghcr.io/cyclonedx/cdxgen-java:{CDXGEN_IMAGE_VERSION}",
                      "java-slim": f"ghcr.io/cyclonedx/cdxgen-java-slim:{CDXGEN_IMAGE_VERSION}",
...
def get_image_for_type(options: Dict, project_type: str | list | None) -> str:
    ...
    return get_env_options_value(options, f"cdxgen_image_{ptype}",
...
def find_cdxgen_cmd(use_bin=True, logger: Logger = None):
    if use_bin:
            ...
            if not os.path.exists(local_bin):
                if logger:
                    logger.info("%s command not found. Please install using npm install "
                                "@cyclonedx/cdxgen or set PATH variable", cdxgen_cmd, )
                return False
        ...
        cdxgen_cmd = local_bin
        # Set the plugins directory as an environment variable
        os.environ["CDXGEN_PLUGINS_DIR"] = (resource_path("local_bin") if sys.platform != "win32" else resource_path(
            os.path.join(lbin,
...
class CdxgenGenerator(XBOMGenerator):
    ...
    def generate(self) -> BOMResult:
        ...
        if not os.getenv("CDXGEN_TEMP_DIR"):
            cdxgen_temp_dir = tempfile.mkdtemp(prefix="cdxgen-temp-", dir=os.getenv("DEPSCAN_TEMP_DIR"))
