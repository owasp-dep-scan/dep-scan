...
- **auto**: Uses the container-image-based cdxgen generator (`CdxgenImageBasedGenerator`) if the Docker command is available. Otherwise, it uses the local cdxgen CLI via `CdxgenGenerator` or the local cdxgen server `CdxgenServerGenerator` if you pass the `--cdxgen-server` argument.
...
- **BlintGenerator**: Uses the optional OWASP blint library. This requires installing depscan with all optional packages:
  ```bash
