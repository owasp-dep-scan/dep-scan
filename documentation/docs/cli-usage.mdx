...
usage: depscan [-h] [--config CONFIG] [--no-banner] [-i SRC_DIR_IMAGE] [-o REPORTS_DIR] [--csaf]
               ...
               [--risk-audit] [--fail-on-error] [--cdxgen-args CDXGEN_ARGS] [--private-ns PRIVATE_NS] [-t PROJECT_TYPE [PROJECT_TYPE ...]] [--bom BOM | --bom-dir BOM_DIR | --purl SEARCH_PURL]
               [--report-template REPORT_TEMPLATE] [--report-name REPORT_NAME] [--deep] [--fuzzy-search] [--search-order {purl,pcu,cpe,cpu,url}] [--severity {low,medium,high,critical}] [--malware-only] [--no-universal]
...
options:
  ...
  --fail-on-error       Abort the scan with a non-zero exit code instead of silently degrading. Passed through to cdxgen so extractor failures fail BOM generation, and applied to blint, rusi, golem, and dosai failures, unreadable reachability reports, empty slices, and empty package lists.
  ...
  --severity {low,medium,high,critical}
                        Minimum severity threshold to report. Only advisories at or above this severity are returned. depscan applies this floor to the final VDR on any database; on the extended vdb (VDB_INCLUDE_METADATA=true) it is additionally pushed down into the search for speed. Findings with no rated severity are kept.
  --malware-only        Report only malware advisories (MAL-*). This works on the default vdb because the is_malware signal is always populated.
...
> [!IMPORTANT]
> Binding dep-scan to a non-local address is authenticated by default. Set `DEPSCAN_SERVER_API_KEY` before using `--server-host 0.0.0.0`, or explicitly opt out with `DEPSCAN_SERVER_ALLOW_UNAUTHENTICATED_BIND=true` only for trusted development environments.
...
## Vulnerability database management (depscan-vdb)

The `depscan-vdb` command selects, downloads, and inspects any published vdb image variant. See the [vulnerability database guide](/vulnerability-database) for the full image matrix.

```text
usage: depscan-vdb [-h] [-v] {download,info,path} ...

Select, download, and inspect any published vdb vulnerability database image
(scope, time, extended tier, compression, distro).

positional arguments:
  {download,info,path}
    download            Download a vdb image into VDB_HOME (default action).
    info                Print information about the locally cached vdb.
    path                Print the VDB_HOME directory.
...
```bash
# Default App+OS database
depscan-vdb download

# Smaller app-only database
depscan-vdb download --scope app

# Extended variant (for severity and metadata search)
depscan-vdb download --scope app --extended

# Distro-specific database
